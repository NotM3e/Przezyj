...
class Game():
    ...
    # Main tab::
        ...
        self.start_button3 = tk.Button(self.start_frame, text="Wczytaj grę!", font=self.font_p1, bg=self.color_2, width=15, command=lambda:
                                       [self.hide_frame(self.start_frame), self.load_frame1.pack(padx=3, fill=tk.BOTH, expand=True)])
    ...
    # Info tab:
        ...
        self.info_frame.pack(padx=3, pady=10, fill=tk.BOTH, expand=True)
    ...
    # Load save tab:
        ...
        self.load_label1 = tk.Label(self.load_frame1, text="Zapisy", font=self.font_h2, fg=self.color_7, bg=self.color_1)
        ...
        self.load_button2 = tk.Button(self.load_frame1, text="Usuń", font=self.font_p1, bg=self.color_2, width=15, command=self.selected_delate_save)
        ...
        self.load_frame1.pack(padx=3, fill=tk.BOTH, expand=True)
        ...
        self.load_question_icon = tk.PhotoImage(file="data/img/questionmark.png")
        self.load_question_canvas = tk.Canvas(self.load_frame2, bg=self.color_1, bd=0, highlightthickness=0, width=32, height=32)
        self.load_question_canvas.create_image(16, 16, anchor=tk.CENTER, image=self.load_question_icon)
        self.load_question_canvas.pack(pady=5, padx=5, side=tk.RIGHT, anchor=tk.SE)
        
        self.load_question_canvas.bind("<Button-1>", lambda event: self.show_info_message("Tytuł", "Treść wiadomości")) # TEMP configuracja wiadomości
    ...
    def load_saves(self, listbox):
        n = 1 # Tymczasowa
        if os.path.exists(f"saves/save{n}.txt"):
            with open(f"saves/save{n}.txt", "r") as file:
                for line in file:
...
# Main game:
    ...
    # Main tab:
        ...
        self.maingame_frame1 = tk.Frame(self.maingame_frame0, bg=self.color_3, highlightbackground=self.color_8, highlightthickness=2)
        self.maingame_stats_label1 = tk.Label(self.maingame_frame1, text="Dzień", font=self.font_p2i, fg=self.color_2, bg=self.color_3)
        self.maingame_stats_label2 = tk.Label(self.maingame_frame1, text="Pieniądze", font=self.font_p2i, fg=self.color_2, bg=self.color_3)
        self.maingame_stats_label3 = tk.Label(self.maingame_frame1, text=self.stats['day'], font=self.font_p1b, fg=self.color_7, bg=self.color_3)
        self.maingame_stats_label4 = tk.Label(self.maingame_frame1, text=f"{self.stats['money']} PLN", font=self.font_p1b, fg=self.color_7, bg=self.color_3)

        # Configure grid columns
        self.maingame_frame1.grid_columnconfigure(0, weight=1)
        self.maingame_frame1.grid_columnconfigure(1, weight=2)
        self.maingame_frame1.grid_columnconfigure(2, weight=0)
        self.maingame_frame1.grid_columnconfigure(4, weight=0)
        self.maingame_frame1.grid_columnconfigure(6, weight=0)

        # Create images
        self.maingame_debug_canvas = tk.Canvas(self.maingame_frame1, bg=self.color_3, bd=0, highlightthickness=0, width=48, height=52)
        self.maingame_debug_canvas.create_image(24, 27, anchor=tk.CENTER, image=self.maingame_debug_icon)
        self.maingame_save_canvas = tk.Canvas(self.maingame_frame1, bg=self.color_3, bd=0, highlightthickness=0, width=48, height=52)
        self.maingame_save_canvas.create_image(24, 27, anchor=tk.CENTER, image=self.maingame_save_icon)
        self.maingame_home_canvas = tk.Canvas(self.maingame_frame1, bg=self.color_3, bd=0, highlightthickness=0, width=48, height=52)
        self.maingame_home_canvas.create_image(24, 27, anchor=tk.CENTER, image=self.maingame_home_icon)
        self.maingame_settings_canvas = tk.Canvas(self.maingame_frame1, bg=self.color_3, bd=0, highlightthickness=0, width=48, height=52)
        ...
        self.maingame_frame2 = tk.Frame(self.maingame_frame0, bg=self.color_1)
        self.maingame_button1 = tk.Button(self.maingame_frame2, text="Następny dzień", font=self.font_p1, bg=self.color_2, width=15, command=self.next_day)
        self.maingame_button2 = tk.Button(self.maingame_frame2, text="Sklep", font=self.font_p1, bg=self.color_2, width=15)
        self.maingame_button3 = tk.Button(self.maingame_frame2, text="Praca", font=self.font_p1, bg=self.color_2, width=15)
        self.maingame_button4 = tk.Button(self.maingame_frame2, text="Statystyki", font=self.font_p1, bg=self.color_2, width=15)

        # Visualisation of statistical elements
        self.maingame_frame0.pack(fill=tk.BOTH, expand=True, padx=3, pady=10)
        self.maingame_frame1.pack(fill="x", side=tk.TOP, anchor=tk.N, ipady=3, ipadx=3)
        ...
        self.maingame_frame2.pack(expand=True)
        ...
        # Question icon
        self.maingame_question_icon = tk.PhotoImage(file="data/img/questionmark.png")
        self.maingame_question_canvas = tk.Canvas(self.maingame_frame0, bg=self.color_1, bd=0, highlightthickness=0, width=32, height=32)
        self.maingame_question_canvas.create_image(16, 16, anchor=tk.CENTER, image=self.maingame_question_icon)
        ...
        self.maingame_question_canvas.bind("<Button-1>", lambda event:
                self.show_info_message("Tytuł", "Treść wiadomości"))    # TEMP configuracja wiadomości
        self.maingame_save_canvas.bind("<Button-1>", lambda event:
                [self.hide_frame(self.maingame_frame1, self.maingame_frame2, self.maingame_question_canvas), self.maingame_save_frame1.pack(padx=3, pady=10, expand=True)])
    ...
    # Save tab:
        ...
        self.maingame_save_listbox = tk.Listbox(self.maingame_save_frame1, selectmode=tk.SINGLE, width=50, height=5)
        self.maingame_save_entry1 = tk.Entry(self.maingame_save_frame1, validate="key", validatecommand=validate_cmd, width=40)
        self.maingame_save_button1 = tk.Button(self.maingame_save_frame1, text="Zapisz", font=self.font_p1, bg=self.color_2, width=15)

        self.maingame_save_frame1.pack(padx=3, pady=10, expand=True)
        self.maingame_save_label1.pack()
        self.maingame_save_listbox.pack(padx=5, pady=5)
        self.maingame_save_entry1.pack()
        self.maingame_save_button1.pack()

        self.maingame_save_frame1.pack_forget()

        # save_title = maingame_save_entry1.get()
        self.load_saves(self.maingame_save_listbox)
